...
def pgd(imgs, epsilon, alpha, iter, model, criterion, labels):
    ...
    # 初始化对抗样本为原图的拷贝
    adv_xs = imgs.clone().detach().float()

    # 记录原始图片用于投影（Project）
    ori_imgs = imgs.clone().detach().float()
    ...
    for i in range(iter):
        ...
        # PGD 核心更新公式：在梯度方向前进一小步（梯度上升）
        # adv_xs = adv_xs + alpha * sign(grad)
        adv_xs = adv_xs.detach() + alpha * grad.sign()

        # Clip perturbation
        # 投影步 (Projection Step)：确保扰动在 epsilon-ball 范围内
        # eta = clamp(adv_xs - ori_imgs, -epsilon, epsilon)
        eta = torch.clamp(adv_xs - ori_imgs, min=-epsilon, max=epsilon)

        # 将扰动加回原图，并限制在图像合法值域 [0, 1] 内
        adv_xs = torch.clamp(ori_imgs + eta, min=0.0, max=1.0)
...
def fgsm_target(imgs, epsilon, model, criterion, labels):
    ...
    # Forward and compute loss, then backward
    outputs = model(adv_xs)
    # 定向攻击：最小化与目标标签 labels 的损失
    loss = criterion(outputs, labels)
    model.zero_grad()
    loss.backward()
    # Retrieve grad and generate adversarial example, note to detach
    grad = adv_xs.grad.data
    # 定向攻击更新公式：沿着梯度负方向更新（梯度下降），使预测结果接近目标标签
    # adv_xs = adv_xs - epsilon * sign(grad)
    adv_xs = adv_xs.detach() - epsilon * grad.sign()
...
def pgd_target(imgs, epsilon, alpha, iter, model, criterion, labels):
    ...
    for i in range(iter):
        ...
        # Retrieve grad and generate adversarial example, note to detach
        grad = adv_xs.grad.data
        # 定向攻击步骤：梯度下降，减小与目标标签的差距
        adv_xs = adv_xs.detach() - alpha * grad.sign()

        # Clip perturbation
        # 投影步：确保扰动在 epsilon 范围内
        eta = torch.clamp(adv_xs - ori_imgs, min=-epsilon, max=epsilon)
        adv_xs = torch.clamp(ori_imgs + eta, min=0.0, max=1.0)
